...
$u_msg=$_POST['usermsg'];
if(trim($name)!="Your Name" && trim($email)!="Your Email" && trim($sub)!="Your Subject" && trim($phone)!="Your Phone" && trim($u_msg)!="Your Message" && trim($name)!="" && trim($email)!="" && trim($sub)!="" && trim($phone)!="" && trim($u_msg)!="")
{
	if(filter_var($email, FILTER_VALIDATE_EMAIL))
	{
		...
		if(mail('support@templatebundle.net','Query for Truelawyer',$message,$headers ))
		{
		mail($email,'Reply from Truelawyer Template Team',$message_user,$headers );
